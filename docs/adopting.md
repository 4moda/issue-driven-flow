...
One secret is enough. When secrets for both agents are present,
auto-detection prefers claude, then codex; pass
`with: { agent: codex }` (or `claude`) to `shape` and `build` to pick one
explicitly.
...
(e.g. `claude-opus-4-8`, `claude-sonnet-5`), Codex ids in its vendor docs.
...
`WebSearch`/`WebFetch` tools and Codex's `web_search`.
...
build). The turn cap is enforced for claude; codex has no such setting,
so only the timeout bounds it. Exceeding either fails the run,
